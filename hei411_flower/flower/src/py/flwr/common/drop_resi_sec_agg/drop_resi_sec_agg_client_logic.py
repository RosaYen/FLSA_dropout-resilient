...
from flwr.common.drop_resi_sec_agg import drop_resi_sec_agg_primitives
...
def ask_keys(client, ask_keys_ins: AskKeysIns) -> AskKeysRes:
    ...
    client.sk1, client.pk1 = drop_resi_sec_agg_primitives.generate_key_pairs()
    client.sk2, client.pk2 = drop_resi_sec_agg_primitives.generate_key_pairs()
    log(INFO, "SecAgg Stage 1 Completed: Created Key Pairs")
    return AskKeysRes(
        pk1=drop_resi_sec_agg_primitives.public_key_to_bytes(client.pk1),
        pk2=drop_resi_sec_agg_primitives.public_key_to_bytes(client.pk2),
    )
...
def share_keys(client, share_keys_in: ShareKeysIns) -> ShareKeysRes:
    ...
    if client.public_keys_dict[client.sec_agg_id].pk1 != drop_resi_sec_agg_primitives.public_key_to_bytes(client.pk1) or client.public_keys_dict[client.sec_agg_id].pk2 != drop_resi_sec_agg_primitives.public_key_to_bytes(client.pk2):
        raise Exception(
            "Own public keys are displayed in dict incorrectly, should not happen!")

    # Generate private mask seed
    client.b = drop_resi_sec_agg_primitives.rand_bytes(32)

    # Create shares
    b_shares = drop_resi_sec_agg_primitives.create_shares(
        client.b, client.threshold, client.share_num
    )
    sk1_shares = drop_resi_sec_agg_primitives.create_shares(
        drop_resi_sec_agg_primitives.private_key_to_bytes(
    ...
    for idx, p in enumerate(client.public_keys_dict.items()):
        ...
        if client_sec_agg_id == client.sec_agg_id:
            ...
            shared_key = drop_resi_sec_agg_primitives.generate_shared_key(
                client.sk2, drop_resi_sec_agg_primitives.bytes_to_public_key(client_public_keys.pk2))
            client.shared_key_2_dict[client_sec_agg_id] = shared_key
            plaintext = drop_resi_sec_agg_primitives.share_keys_plaintext_concat(
                client.sec_agg_id, client_sec_agg_id, b_shares[idx], sk1_shares[idx])
            ciphertext = drop_resi_sec_agg_primitives.encrypt(shared_key, plaintext)
...
def ask_vectors(client, ask_vectors_ins: AskVectorsIns) -> AskVectorsRes:
    ...
    for packet in packet_list:
        ...
        plaintext = drop_resi_sec_agg_primitives.decrypt(shared_key, ciphertext)
        try:
            plaintext_source, plaintext_destination, plaintext_b_share, plaintext_sk1_share = drop_resi_sec_agg_primitives.share_keys_plaintext_separate(
                plaintext)
        except:
            raise Exception(
    ...
    quantized_weights = drop_resi_sec_agg_primitives.quantize(
    ...
    quantized_weights = drop_resi_sec_agg_primitives.weights_multiply(
        quantized_weights, weights_factor)
    quantized_weights = drop_resi_sec_agg_primitives.factor_weights_combine(
        weights_factor, quantized_weights)

    dimensions_list: List[Tuple] = [a.shape for a in quantized_weights]

    # add private mask
    private_mask = drop_resi_sec_agg_primitives.pseudo_rand_gen(
        client.b, client.mod_range, dimensions_list)
    quantized_weights = drop_resi_sec_agg_primitives.weights_addition(
        quantized_weights, private_mask)

    for client_id in available_clients:
        # add pairwise mask
        shared_key = drop_resi_sec_agg_primitives.generate_shared_key(
            client.sk1, drop_resi_sec_agg_primitives.bytes_to_public_key(client.public_keys_dict[client_id].pk1))
        pairwise_mask = drop_resi_sec_agg_primitives.pseudo_rand_gen(
            shared_key, client.mod_range, dimensions_list)
        if client.sec_agg_id > client_id:
            quantized_weights = drop_resi_sec_agg_primitives.weights_addition(
                quantized_weights, pairwise_mask)
        else:
            quantized_weights = drop_resi_sec_agg_primitives.weights_subtraction(
                quantized_weights, pairwise_mask)

    # Take mod of final weight update vector and return to server
    quantized_weights = drop_resi_sec_agg_primitives.weights_mod(
