...
def sec_agg_fit_round(server, rnd: int
    ...
    masked_vector = drop_resi_sec_agg_primitives.weights_zero_generate(
    ...
    for idx, client in ask_vectors_clients.items():
        if client in [result[0] for result in ask_vectors_results]:
            ...
            masked_vector = drop_resi_sec_agg_primitives.weights_addition(
    ...
    for client_id, share_list in collected_shares_dict.items():
        ...
        secret = drop_resi_sec_agg_primitives.combine_shares(share_list=share_list)
        if client_id in unmask_vectors_clients.keys():
            ...
            private_mask = drop_resi_sec_agg_primitives.pseudo_rand_gen(
                secret, sec_agg_param_dict['mod_range'], drop_resi_sec_agg_primitives.weights_shape(masked_vector))
            masked_vector = drop_resi_sec_agg_primitives.weights_subtraction(
            ...
            for neighbor_id in neighbor_list:
                shared_key = drop_resi_sec_agg_primitives.generate_shared_key(
                    drop_resi_sec_agg_primitives.bytes_to_private_key(secret), drop_resi_sec_agg_primitives.bytes_to_public_key(public_keys_dict[neighbor_id].pk1))
                pairwise_mask = drop_resi_sec_agg_primitives.pseudo_rand_gen(
                    shared_key, sec_agg_param_dict['mod_range'], drop_resi_sec_agg_primitives.weights_shape(masked_vector))
                if client_id > neighbor_id:
                    ...
                    masked_vector = drop_resi_sec_agg_primitives.weights_subtraction(
                        masked_vector, pairwise_mask)
    masked_vector = drop_resi_sec_agg_primitives.weights_mod(
    ...
    total_weights_factor, masked_vector = drop_resi_sec_agg_primitives.factor_weights_extract(
        masked_vector)
    masked_vector = drop_resi_sec_agg_primitives.weights_divide(
        masked_vector, total_weights_factor)
    aggregated_vector = drop_resi_sec_agg_primitives.reverse_quantize(
